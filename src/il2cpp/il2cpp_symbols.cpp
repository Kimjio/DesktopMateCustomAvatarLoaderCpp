...
namespace il2cpp_symbols
{
	...
	vector<const char*> symbol_real_names;

	void load_symbols()
	{
		int startRva = 0x76ca92;

		int rva = startRva;

		try
		{
			auto file = CreateFileW(L"UnityPlayer.dll", GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
#define DO_API_NO_RETURN(r, n, p) symbol_names.emplace_back(#n)
#include "il2cpp-api-functions.h"
...
#undef DO_API
			...
			for (auto name : symbol_names)
			{
				...
				symbol_real_names.emplace_back(real_name);
	...
	{
		int i = 0;
#define DO_API(r, n, p) n = reinterpret_cast<decltype(n)>(GetProcAddress(game_module, symbol_real_names[i])); i++;
		// PROFILER is only used in debug builds
#undef IL2CPP_ENABLE_PROFILER
#define IL2CPP_ENABLE_PROFILER 0
#define DO_API_NO_RETURN(r, n, p) n = reinterpret_cast<decltype(n)>(GetProcAddress(game_module, symbol_real_names[i])); i++;
#include "il2cpp-api-functions.h"
#undef DO_API_NO_RETURN
#undef IL2CPP_ENABLE_PROFILER
#define IL2CPP_ENABLE_PROFILER !IL2CPP_TINY
